...
app.post('/auth/register', function(req, res){
  ...
  //Setup mailer transport, I chose gmail. Create an application-specific password to avoid problems.
  ...
  pool.getConnection(function(err, connection){
    ...
    connection.query(sql, [uname], function(err, result){
      if(err){
        ...
        if(result[0].email === req.body.email && uname === result[0].username){
          var mailOpts, smtpTrans;
          //Setup mailer transport, I chose gmail. Create an application-specific password to avoid problems.
          // create reusable transporter object using the default SMTP transport
          ...
          let transporter = nodemailer.createTransport({
            ...
            let mailOptions = {
              from: 'fkam12@naver.com', //grab form data from the request body object
              to: req.body.email,
              subject: '[소통박스] 인증을 클릭하여 비밀번호를 찾으세요!',
              // text: req.body.message,
              html :"<h1>안녕하세요! "+req.body.username+" 님!</h1>"+"<br>"
              +"<a href='http://localhost:3003/findpass/"+ req.body.username +'/confirm'+ "'>인증</a>" +
              "을 해주세요!"
          // '<a href="localhost:3003/auth/'+ req.body.name +'/'+ rand + '">인증</a>'
            };
...
app.get('/findpass/:username/confirm', function(req, res){
...
app.post('/findpass/:username/confirm', function(req, res){
  var uname = req.params.username;
...
app.post('/emailConfirm', function(req, res){
  var ajax_email = req.body.email;
  console.log(ajax_email);

  pool.getConnection(function(err, connection){
    var sql = "select count(email) as e_num from users where email=?";
...
app.post('/displayNameConfirm', function(req, res){
